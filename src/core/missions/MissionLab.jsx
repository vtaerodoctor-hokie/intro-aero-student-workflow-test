...
export default function MissionLab(){
 ...
 return <main className="mission-shell">
  ...
  <section className="mission-system" aria-label="Aircraft system status"><span>Week 5 baseline: {data.baseline?'preserved':'not installed'}</span><span>Pitch authority: {status.flightUnlocked?'verified':'locked'}</span><span>Roll / yaw: outside this mission</span><span>Takeoff trajectory: not modeled</span></section>
  ...
  <p>{teaching?.notice||'No private mission preset is installed. The workflow and blank scaffolds remain available.'}</p>
